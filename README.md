...
**Question for Class Discussion**: How does the shift short attention mechanism help with context tokens? What advantages does it offer?
   ...
   - In standard self-attention, each token in the input sequence attends to all other tokens, including itself. This results in a full attention matrix where the attention weights are computed      based on the similarity between the query, key, and value representations of tokens. The computational complexity of standard self-attention is quadratic (O(n²)) with respect to the            sequence length, making it computationally expensive for very long sequences.
   ...
   - Let’s say you want to train a model with an 8k context window. Input could be split into multiple parts with maximal lengths of 2k tokens as used for classic fine-tuning:
```
1, 2, …, 2047, 2048
2049, 2050, …, 4095, 4096
4097, 4098, …, 6143, 6144
